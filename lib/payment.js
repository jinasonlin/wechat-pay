...
Payment.prototype.getWXPayRequestParams =
Payment.prototype.getBrandWCPayRequestParams = function (order, callback) {
  var self = this;
  var default_params = {};
  if (order.trade_type == 'JSAPI') {
    default_params.appId = this.appId;
    default_params.timeStamp = this._generateTimeStamp();
    default_params.nonceStr = this._generateNonceStr();
    default_params.signType = 'MD5';
  } else if (order.trade_type == 'APP') {
    default_params.appid = this.appId;
    default_params.partnerid = this.mchId;
    default_params.timestamp = this._generateTimeStamp();
    default_params.noncestr = this._generateNonceStr();
  }
  ...
  this.unifiedOrder(order, function (err, data) {
    ...
    var params = _.extend(default_params, _params);

    params.sign = params.paySign = self._getSign(params);
...
Payment.prototype._signedQuery = function(url, params, options, callback){
  ...
  params = this._extendWithDefault(params, options.params || [
    'appid',
    'mch_id',
    'sub_mch_id',
    'nonce_str'
...
Payment.prototype.validate = function (xml, callback) {
  ...
  xml2js.parseString(xml, {
    ...
    } else if (this instanceof Payment && self._getSign(data) !== data.sign) {
      error = new Error();
